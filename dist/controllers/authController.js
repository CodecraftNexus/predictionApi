    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            const responseJson = {
                success: true,
                message: "Login successful",
                requiresUsername: isNewUser || (user === null || user === void 0 ? void 0 : user.username),
                user: {
                    id: user.id,
                    name: user.name,
                    email: user.email,
...
function setUsername(req, res) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            const existing = yield db_1.db.User.findOne({ where: { username } });
            if (existing) {
                return res.status(400).json({ success: false, message: "Username already taken" });
            }
            const userId = req.user.userId;
            const user = yield db_1.db.User.findByPk(userId);
            yield (user === null || user === void 0 ? void 0 : user.update({ username }));
            return res.json({
                success: true,
                user: Object.assign(Object.assign({}, user === null || user === void 0 ? void 0 : user.toJSON()), { username }),
            });
        }
        catch (error) {
